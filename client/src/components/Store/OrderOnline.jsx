...
import ProductList from "../OrderOnline/ProductList";
import MenuListContainer from "../OrderOnline/MenuListContainer";

// redux
import { useSelector, useDispatch } from "react-redux";
import { getProductList } from "../../redux/reducers/Product/Product.action";
...
const OrderOnline = () => {
  ...
  const Store = useSelector(
    (globaldata) => globaldata.Store.selectedStore.Store
  );

  useEffect(() => {
    Store &&
      dispatch(getProductList(Store.menu)).then((data) => {
      ...
      <div className="w-full flex items-start">
        ...
        <div className="w-full px-3 md:w-3/4 sticky overflow-auto h-screen top-16">
          ...
          <section className="flex  overflow-y-screen flex-col gap-3 md:gap-5">
            {menu.map((item, index) => (
              <ProductList key={index} {...item} target={index} />
            ))}
          </section>
        </div>
      </div>
